...
Planning baseline: 2026-09-23. **Bootstrap, data/imports, the bilingual UI foundation, and authentication with role-based access (D23) are merged into `main`; from milestone 5 Claude Code implements at the applicant's direction (D24). Quiz authoring is in progress on `feat/quiz-authoring`: teachers can create drafts, edit settings and questions, and publish with an availability window (D25). On `feat/timed-attempts`, students can start and resume a timed attempt and save answers on the server (D26). Submission, grading, and reports remain pending.** Unless a result is explicitly recorded, verification items describe intended evidence, not passing tests.
