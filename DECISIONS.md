...
Planning baseline: 2026-09-23. **Bootstrap, data/imports, the bilingual UI foundation, and authentication with role-based access (D23) are merged into `main`; from milestone 5 Claude Code implements at the applicant's direction (D24). Quiz authoring is in progress on `feat/quiz-authoring`: teachers can create drafts, edit settings, and edit questions (D25). Publication, attempts, grading, and reports remain pending.** Unless a result is explicitly recorded, verification items describe intended evidence, not passing tests.
...
**Trade-off:** After a validation error the form shows the message but reloads saved values, so a teacher retypes the rejected field; browser-side limits make this rare. Deleting a draft and reordering questions (other than by deleting) are not offered in the core.
