...
The server and migration CLI use the same normalized file path. Application connections explicitly enable foreign keys and WAL mode, with a bounded 5-second busy timeout; SQLite still permits only one writer. Keep a single shared Prisma client per web process and short transactions. Milestone 7 testing confirmed why: better-sqlite3 waits for a lock synchronously, so two connections that both open transactions inside one Node process block each other until the busy timeout, while one shared client runs its transactions one after another. Separate processes (such as the import CLI next to the server) simply wait for the lock. The readiness endpoint queries a real application table and returns 503 when that fails.
...
**Questions (part 2):** On a draft, the owner adds, edits, and deletes questions: text (up to 500 characters, line breaks allowed), points (0.01–1,000, two decimals, stored as hundredths), four different non-empty options, and one correct option chosen with a radio button. These are the importer's limits (D21), so a browser-made and an imported question are interchangeable. New questions are appended; deleting one renumbers the rest so positions stay consecutive, which publication will require. Every change first runs the same owner-and-draft conditional update on the quiz inside its transaction, so questions can never change on a published quiz, and a question is only found through its own quiz (another quiz's question id is reported as missing). A quiz holds at most 200 questions. Question and option text uses automatic direction, so Arabic and English content each display correctly in either interface language. The teacher's own page lists the correct answers; students never receive them (D06).

**Publication (part 3):** The owner publishes a draft by choosing opening and closing times, entered on an Amman wall clock (`datetime-local`) and stored as UTC instants. The conversion uses the time-zone rules for that date, so it stays correct across Jordan's historic offset changes; impossible or malformed times are rejected. The form suggests a week from the next five-minute mark and requires an explicit checkbox confirming that the quiz becomes fixed, which works without JavaScript. The server then rejects: a window that has already closed, a closing time not after the opening time, a window shorter than the quiz duration (so a student who starts at opening can use the full time), a quiz with no questions, any incomplete question (positions not consecutive, not exactly four non-empty options, missing points or correct option), and any assigned class the teacher no longer teaches. Publication takes the same owner-and-draft lock first, validates the stored content inside that transaction, and only then sets `PUBLISHED` with the window, so no edit can slip in between validation and publication. After that, settings and questions return "locked", and the quiz appears on the home page of students in its classes with its window status (D05).

**Trade-off:** After a validation error the form shows the message but reloads saved values, so a teacher retypes the rejected field; browser-side limits make this rare. Deleting a draft, reordering questions (other than by deleting), unpublishing, and changing a published window are not offered in the core; a changed assessment is a new quiz (D04).

**Verification:** Real-SQLite tests cover accepted boundaries and each invalid field, drafts hidden from students, untaught and mixed class lists (no write), owner edits including class replacement, another teacher's quiz (unchanged, not found), a published quiz (unchanged, locked), and full rollback when one class is not taught. HTTP smoke checks cover the offered classes, 404 for another teacher, 403 for a student, sign-in redirect, a locked published quiz, and an untaught class; with `SMOKE_WRITES=1` (CI) they also create and edit a draft. Question tests cover validation limits and line-ending normalization, appending with four ordered options, refusal for other teachers and published quizzes (no writes), full updates, cross-quiz question ids, renumbering after deletion with options removed, and the 200-question limit; smoke checks cover 403/404 and locked responses for all three question actions and read-only published questions, plus (with writes) adding, editing, and deleting a question. Publication tests cover Amman conversion with both the current UTC+3 and the historic UTC+2 offsets, malformed and impossible times, the suggested window, confirmation and window rules, a successful publication that becomes visible to its class at the right time and then refuses edits, another teacher's draft, empty and incomplete drafts, an untaught class, and the duration boundary; smoke checks cover 403/404, missing confirmation, a locked published quiz, and (with writes) a published quiz appearing only for its class.

## D26 - Starting and resuming a timed attempt

**Basis:** "Students log in, take a timed multiple choice quiz" and "should not be able to take a quiz twice" (brief); D05 fixes the timing rules.

**Decision:** A student opens a quiz from their list and sees its rules before starting: one attempt, the timer keeps running if the page or device closes, the deadline rule, the marking rule, and the time they would actually get if they started now (with a warning when the quiz closes before the full duration). Starting is a form post; the server checks that the quiz is published, assigned to the student's own class, and open on server time, then stores the attempt with `deadlineAt = min(start + duration, closesAt)`. Drafts and other classes' quizzes return 404, like missing ones.

Starting is idempotent. If the student already has an attempt in any state, that attempt is returned unchanged, so a refresh, a second tab, a double click, or signing in again resumes it without moving its deadline. Two simultaneous first starts are settled by the unique (student, quiz) index; the losing request reads the winning attempt. Attempt pages address the quiz, not an attempt id, and always load the signed-in student's own attempt.

The attempt page receives the questions and options but not the correct option; the query never selects it (D06). Its countdown starts from the server's remaining time and counts with the browser's monotonic clock, so changing the device clock does not add time; at zero the page reloads to show the server's state. The deadline is also shown as an Amman time.

**Saving answers (part 2):** Each question is a small form. Without JavaScript, choosing an option and pressing **Save answer** posts it and returns to that question. With JavaScript, choosing an option saves it immediately and shows "saving", "saved", or "not saved" with a retry button; only the server's acknowledgment counts as saved (D08). A **Clear answer** button removes a saved answer, which matters when wrong answers are penalized. Saves for one question are sent one at a time, and only the newest pending choice is sent next, so a slow earlier request cannot overwrite a newer answer. The server's first statement is a conditional update of the student's own attempt (in progress and before the deadline on the server clock) that also increments its version. It takes the write lock, so expiry or a later submission cannot interleave. Then it checks that the question belongs to the quiz and the option to that question (also enforced by composite foreign keys, D17). Invalid requests roll back entirely. At or after the deadline, or on a finished attempt, the answer is refused and the page reloads to show the server's state.

**Trade-off / scope:** The countdown can lag the server by the page's network delay; the server's deadline is what counts. An attempt whose time has run out is shown as over but is not finalized until grading is implemented (D07, milestone 9). Pending answers are kept only in the open page: there is no local-storage queue, automatic background retry, or cross-tab conflict message (the rest of D08). If the connection fails, the student sees "not saved" and can retry while time remains.

**Verification:** Real-SQLite tests cover the deadline rule (full duration and capped at closing), creation with a persisted deadline and an unchanged resume, a late starter getting only the remaining time, refusal for another class, a draft, before opening, and at the closing instant (no rows written), an already submitted attempt being returned rather than duplicated, two simultaneous starts on separate connections producing one attempt, and attempt data without any answer key or another student's attempt. Smoke checks cover class scoping, 403 for a teacher, the sign-in redirect, and (with `SMOKE_WRITES=1`) starting twice without changing the deadline. Answer tests cover saving, replacing, and clearing one answer per question with an increasing version, restoring answers on reopening, options from another question or quiz and malformed ids (nothing written, version rolled back), the last millisecond before the deadline versus the deadline itself, a finished attempt and a student without an attempt, and two simultaneous saves on the shared connection leaving exactly one answer. Smoke checks cover 404 without an attempt, 403 for a teacher and for a cross-site post, the sign-in redirect, and (with writes) JSON and plain-form saves, a wrong-question option, clearing, and answers surviving a reload.

## D27 - Submission, grading, and results

**Basis:** Students "see their score at the end"; Nour wants "to see how the students did"; negative marking depends on the teacher and quiz (brief). D06/D07 fix the rules.

**Decision:** A student submits from the attempt page. Grading runs only on the server, from answers saved before the deadline, inside the same kind of locked transaction as saving: SUBMITTED if submitted before the deadline, EXPIRED otherwise. An attempt whose deadline passed without submission is finalized from its saved answers the next time the student, the teacher, or the administrator views it, so no report shows an overdue attempt as in progress and no scheduler is needed. Finalizing is idempotent: a repeated submission returns the stored result unchanged, and saving answers afterwards is refused.

**Precision policy:** A correct answer earns its points; a wrong one loses `penalty%` of that question's points; a blank earns zero. The total is computed exactly (hundredths of a point x 10,000), rounded once to hundredths (half up), and only then floored at zero, so negative marking never produces a negative score and is not floored per question. Scores are stored as integer hundredths.

**Results:** The student sees the score out of the maximum and the counts of correct, incorrect, and unanswered questions, but not the answer key (D06). The teacher's published quiz page lists every student in its classes, including those who have not started, with status and score, plus totals and an average percentage; only the owner can see it. The administrator's overview lists all published quizzes, each linking to the same centre-wide results table.

**Scope (time-boxed):** The in-page save with retry from D26 is the delivered recovery; a local-storage queue for offline answers, cross-tab conflict messages, and result export remain future work.

**Verification:** Hand-calculated tests cover the seed's 7.75/18 example, the zero floor applied to the total only, blanks, half-up rounding (0.625 -> 0.63), and zero penalty; database tests cover a single stored grade on repeated submission, refusal to save afterwards, expiry after the deadline (and not before), owner-only teacher results, a draft having no results, non-starters, and the average. Smoke checks cover owner/admin access, a teacher refused from the admin page, submission without an attempt, and (with writes) submission, the result display, a late save refused, and a repeated submission.
