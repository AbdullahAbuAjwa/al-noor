...
**Current status: sign-in and role-based access.** The application has persistent SQLite storage, automatic first-use demo data, CSV/XLSX command-line imports, a responsive Arabic/English interface, and sign-in for students, teachers, and the centre administrator. Each role lands on its own page with data scoped to that account, and teachers can create quiz drafts, edit their settings and questions, and publish them to their classes. Taking a quiz, taking a quiz, grading, and detailed reports are not available yet.
