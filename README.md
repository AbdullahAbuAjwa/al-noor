...
**Current status: sign-in and role-based access.** The application has persistent SQLite storage, automatic first-use demo data, CSV/XLSX command-line imports, a responsive Arabic/English interface, and sign-in for students, teachers, and the centre administrator. Each role lands on its own page with data scoped to that account, and teachers can create quiz drafts and edit their settings. Adding questions in the browser, publishing, taking a quiz, grading, and detailed reports are not available yet.
...
- **Teacher** (`/teacher`): only that teacher's quizzes and drafts, with classes, window, settings, and the number of finished attempts. **New quiz** creates a draft (title, the teacher's own classes, duration, wrong-answer penalty as a percentage); opening a draft lets its owner edit those settings. Published quizzes open read-only. Question editing and publishing arrive with the next parts of this milestone.
